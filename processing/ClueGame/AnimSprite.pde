...
//~~~~~~~~~~~~~~~~~~~~~~
  ...
  void draw() {
    ...
    if(!isTexture){
      pg.imageMode(CENTER);
      pg.image(frames[frameIndex], 0, 0);
    }else{
      ...
      pg.noStroke();
      pg.beginShape(QUADS);
      pg.texture(frames[frameIndex]);
      pg.vertex(vertices[0].x,vertices[0].y,uvs[0].x,uvs[0].y);
      pg.vertex(vertices[1].x,vertices[1].y,uvs[1].x,uvs[1].y);
      pg.vertex(vertices[2].x,vertices[2].y,uvs[2].x,uvs[2].y);
      pg.vertex(vertices[3].x,vertices[3].y,uvs[3].x,uvs[3].y);
      pg.endShape(CLOSE);
      if(debug){
        pg.noStroke();
        pg.ellipseMode(CENTER);
        pg.fill(255,0,0);
        pg.ellipse(vertices[0].x,vertices[0].y,5,5);
        pg.fill(0,255,0);
        pg.ellipse(vertices[1].x,vertices[1].y,5,5);
        pg.fill(0,0,255);
        pg.ellipse(vertices[2].x,vertices[2].y,5,5);
        pg.fill(255,0,255);
        pg.ellipse(vertices[3].x,vertices[3].y,5,5);
      }
    }
    pg.popMatrix();
  }
  ...
  void rotateXYZ(float _x, float _y, float _z) {
    pg.rotateX(radians(_x));
    pg.rotateY(radians(_y));
    pg.rotateZ(radians(_z));
  }
  ...
  void blendImage(PImage bI, int pX, int pY, String b, boolean center) {
    ...
    for (int i=0;i<blendModes.length;i++) {
      if (b==blendModes[i]) {
        ...
        pg.blend(bI, 0, 0, bI.width, bI.height, qX, qY, bI.width, bI.height, blendModeCodes[i]);
      }
    }
  }
